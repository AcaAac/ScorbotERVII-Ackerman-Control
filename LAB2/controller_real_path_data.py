...
class car:
    ...
    def update(self, v, ws, dt):
        ...
        self.x = self.last_x + (self.dx + self.last_v*np.cos(self.last_theta))* dt
        
        self.dy = v * np.sin(self.theta)
        self.y = self.last_y + (self.dy + self.last_v*np.sin(self.last_theta))* dt
...
def main():
    ...
    Auto = car(x[0], y[0], math.atan2(y_aux[1]-y[0], x_aux[1]-x[0])    , 0,  2.46,  0.03 ,  65  , 3   ,   dt, velocity, ws)
    SSE_control=0
    print("Updating....")
